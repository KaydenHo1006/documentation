...
When creating a new pricelist, start by adding a name for the pricelst at the top of the form, in
...
Under the :guilabel:`Min. Quantity` column, designate if there should be a minimum amount of the
product purchased, in order to apply the specific price.
...
And lastly, if working in a multi-company environment, select to which company this price rule
should be applied in the :guilabel:`Company` field. Leaving this field blank means the price rule
