...
First, in the main *Expenses* app dashboard view (this view can also be accessed from
...
Enter the internal reference in the field. Beneath the :guilabel:`Internal Reference` field, this
sentence appears: :guilabel:`Use this reference as a subject prefix when submitting by email.`
...
categories that filters can be applied on are :guilabel:`Status`, :guilabel:`Employee`, and
:guilabel:`Company`. To view only expense reports with a particular status, click the checkbox next
to the status to display the expense reports with that status. Click the checkbox again to hide the
reports with that status. To view expense reports for a particular employee and/or company, click on
the employee name and/or company name in the :guilabel:`Employee` and :guilabel:`Company` sections.
...
.. important::
   ...
   :guilabel:`Submitted` filter checked off.
...
so that only the :guilabel:`Approved` status is active (checked off).
...
:guilabel:`Memo` field appear. Select the employee's bank account from the drop-down menu to direct
deposit the payment to their account. When all other selections are correct, click :guilabel:`Create
Payment` to send the payment to the employee.
...
.. important::
   ...
   what causes automatic invoicing after an expense report is approved. The :guilabel:`Customer to
   Reinvoice` field can be modified *until an expense report is* **approved**, then the field is no
...
missing, assign the correct account(s) from the drop-down menu, then click :guilabel:`Approve` or
:guilabel:`Refuse`.
...
:guilabel:`Sales Order` smart button to open the :abbr:`SO (Sales Order)` and the expenses to be
re-invoiced are now on it.
...
.. note::
   ...
   :abbr:`SO (Sales Order)` is listed, the :guilabel:`Sales Order` smart button will list the number
   of :abbr:`SO (Sales Order)`'s. If multiple :abbr:`SO (Sales Order)`'s are listed, the smart
   button opens a list view of all the :abbr:`SO (Sales Order)`'s. Click on a :abbr:`SO (Sales
   Order)` to open the individual :abbr:`SO (Sales Order)`.
